-- AdminLess Fin — ERP Blueprint V3.0, Phase 3D: widen the Posting Engine to
-- accept module = 'payroll'. Both the posting_requests CHECK constraint AND
-- posting_engine_submit's internal whitelist must be updated together
-- (Phase 3A lesson: missing the CHECK update caused 23514 failures).
...
COMMENT ON FUNCTION public.posting_engine_submit IS
  'ERP V3.0 Phase 3D: adds payroll module. Modules: sales_invoice, inventory_receipt, inventory_issue, manual_journal, accounts_payable, fixed_assets, banking, payroll.';
