-- AdminLess Fin — ERP Blueprint V3.0, Phase 3B: widen the Posting Engine to
-- accept module = 'banking' (Bank & Cash Management), following the exact
-- same pattern used in Phase 3A to add 'accounts_payable'/'fixed_assets'.
-- Both the posting_requests CHECK constraint AND posting_engine_submit's own
-- internal validation must be updated together (Phase 3A's bug: missing this
-- constraint update caused every AP posting to fail with 23514 until caught
-- by live testing) — this migration fixes both in one step, pre-emptively.
...
ALTER TABLE public.posting_requests ADD CONSTRAINT posting_requests_module_check
  CHECK (module IN (
    ...
    'accounts_payable', 'fixed_assets', 'banking'
  ));
...
BEGIN
  ...
  LOOP
    ...
    -- banking -> protected bank/cash accounts, etc.) is never blocked by its
    -- own systematic postings.
...
COMMENT ON FUNCTION public.posting_engine_submit IS
  'ERP V3.0 Phase 3B: adds banking module. Modules: sales_invoice, inventory_receipt, inventory_issue, manual_journal, accounts_payable, fixed_assets, banking.';
