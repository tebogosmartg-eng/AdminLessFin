-- ============================================================================
-- AdminLess Fin — the facts must include the payroll mappings.
--
-- accounting_facts() reported which control-account ROLES the chart carries,
-- but the payroll clearing requirement is also satisfied by an active payroll
-- account mapping, which the readiness rule has always honoured. Without this
-- fact the composition would have to go back to the tables for that one value,
-- which is exactly the second source the facts layer exists to remove.
-- ============================================================================
  ...
  RETURN jsonb_build_object(
    ...
    'payroll', jsonb_build_object(
      'active_mapping_count', (
        SELECT count(*) FROM public.payroll_account_mappings m
        WHERE m.company_id = p_company_id AND COALESCE(m.is_active, true))),
