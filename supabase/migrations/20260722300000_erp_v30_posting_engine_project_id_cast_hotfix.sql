-- AdminLess Fin — ERP Blueprint V3.0, Phase 3 hotfix: the prior migration
-- (20260722260000) dropped the ::uuid cast on project_id in the
-- journal_entry_items INSERT (a copy/paste regression), causing every
-- posting with a text-vs-uuid mismatch to fail at commit — caught by live
-- testing immediately after deploy. Re-applies the full function with the
-- cast restored; identical otherwise.
