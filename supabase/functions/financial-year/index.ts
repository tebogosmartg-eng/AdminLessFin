...
serve(withEnterprisePlatform('financial-year', 'tenant', async (req, _ctx) => {
  ...
  try {
    ...
    switch (method) {
      ...
      case 'REOPEN':
        ({ error } = await supabaseAdmin.rpc('reopen_financial_year', { p_closed_year_id: body.closed_year_id }));
