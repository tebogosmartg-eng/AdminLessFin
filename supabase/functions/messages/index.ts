...
serve(withEnterprisePlatform('messages', 'tenant', async (req, _ctx) => {
  ...
  try {
    ...
    const { method, company_id, content } = body;
    ...
    switch (method) {
      case 'GET': {
        ...
        data = (messages || []).map((message) => ({
          ...message,
          profiles: profileMap[message.user_id] || null,
        }));
        break;
      }
      ...
      case 'POST': {
        ...
        const { data: inserted, error: insertError } = await supabaseAdmin
          .from('messages')
          .insert({ content: content.trim(), user_id: user.id, company_id })
          .select('*')
          .single();
        ...
        data = { ...inserted, profiles: profile };
        break;
      }
