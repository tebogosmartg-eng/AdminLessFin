...
const RecurringInvoiceForm = ({ isOpen, setIsOpen, invoiceId }: Props) => {
  ...
  const { data: existingData } = useQuery({
    queryKey: ['recurring_invoice_edit', invoiceId],
    queryFn: async () => {
      const { data, error } = await supabase.functions.invoke('recurring-invoices', {
        body: { method: 'GET_ONE', company_id: activeCompany!.id, id: invoiceId },
      });
  ...
  const mutation = useMutation({
    mutationFn: async (values: FormValues) => {
      ...
      const { error } = await supabase.functions.invoke('recurring-invoices', { body });
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>Automate invoice generation for this customer.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="profile_name" render={({ field }) => (<FormItem><FormLabel>Profile Name</FormLabel><FormControl><Input placeholder="e.g. Monthly Retainer" {...field} /></FormControl><FormMessage /></FormItem>)} />
            ...
            <div className="space-y-2 pt-4">
              ...
              {fields.map((field, index) => (
                <div key={field.id} className="grid grid-cols-12 gap-2 items-start">
                  ...
                  <FormField control={form.control} name={`items.${index}.unit_price`} render={({ field }) => (<FormItem className="col-span-2"><FormControl><Input type="number" step="0.01" placeholder="Price" {...field} /></FormControl></FormItem>)} />
                  <FormField control={form.control} name={`items.${index}.income_account_id`} render={({ field }) => (<FormItem className="col-span-2"><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Acc" /></SelectTrigger></FormControl><SelectContent>{incomeAccounts?.map(a => <SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}</SelectContent></Select></FormItem>)} />
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => append({ description: '', quantity: 1, unit_price: 0, income_account_id: '' })}>Add Line</Button>
            </div>
            <DialogFooter>
