...
export default function VendorCreditDocumentView({ model }: { model: VendorCreditDocumentModel }) {
  const identity = [
    model.company.name,
    ...
    >
      ...
      >
        ...
        <div className="text-right">
          <p className="text-3xl font-bold tracking-tight">SUPPLIER CREDIT</p>
      ...
      <div className="space-y-7 px-5 py-7 sm:px-8">
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <dl className="space-y-1.5 text-sm">
            <Row label="Credit date" value={day(model.date)} />
            {model.originalBill && <Row label="Credits bill" value={model.originalBill.number} />}
            {model.originalBill?.date && <Row label="Bill date" value={day(model.originalBill.date)} />}
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          >
            ...
            <p className="mt-2 text-base font-semibold">{model.vendor.name}</p>
            <div className="mt-1 space-y-0.5 text-sm" style={{ color: PAPER.muted }}>
              {model.vendorLines.length === 0 ? (
                <p>No address or contact details are on file for this supplier.</p>
              ) : (
                model.vendorLines.map((line, i) => <p key={i}>{line}</p>)
              )}
            </div>
          </section>
          ...
          >
            <h2 className="text-xs font-bold uppercase tracking-wider">
              {model.isVoid ? 'Supplier credit void' : 'Total credit'}
            </h2>
              ...
              {model.lines.length === 0 && (
                <tr style={{ borderBottom: `1px solid ${PAPER.hairline}` }}>
                  <td className="px-4 py-3" colSpan={columns} style={{ color: PAPER.muted }}>
                    No lines recorded on this credit
                  </td>
                </tr>
              )}
        ...
        <div className="grid gap-6 md:grid-cols-[1fr_auto]">
          ...
          >
            ...
            >
              <ReceiptText className="h-3.5 w-3.5" /> Set off against
            </h2>
            <dl className="mt-3 space-y-1 text-sm">
              {model.applications.length === 0 ? (
                <p style={{ color: PAPER.muted }}>
                  {model.isVoid ? 'Nothing. This credit is void.' : 'Not yet set off against a bill.'}
                </p>
              ) : (
                model.applications.map((a, i) => (
                  <div key={i} className="flex items-baseline gap-3">
                    <dt className="font-semibold">{a.billNumber}</dt>
                    <dd className="flex-1" style={{ color: PAPER.muted }}>
                      {a.billDate ? day(a.billDate) : ''}
                    </dd>
        ...
        >
          {vendorCreditSettlementWording(model)}
        </p>
        ...
        >
          <span>{identity}</span>
          <span>Supplier credit {model.number}</span>
        </footer>
      </div>
