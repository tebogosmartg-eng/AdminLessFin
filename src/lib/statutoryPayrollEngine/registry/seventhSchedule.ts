...
import type { FringeBenefitInput, StatutoryRuleSet } from '../types';
...
export function calculateFringeBenefitLine(
  ...
  switch (benefit.type) {
    ...
    case 'employer_accommodation': {
      ...
      const furnishedMultiplier = ruleSet.furnishedAccommodationAbatementMultiplier;
      if (benefit.furnished && !Number.isFinite(furnishedMultiplier)) {
        throw new Error(
          'Furnished accommodation abatement multiplier missing from tax year legislation. Silent legislative constant fallback is prohibited.'
        );
      }
      const abatement = benefit.furnished
        ? ruleSet.accommodationAbatementAnnual * furnishedMultiplier
      ...
      return {
        ...
        taxableValue: taxable,
        legislativeReference: 'Seventh Schedule para 9 — rental value minus abatement',
        auditTrail: [
          createAuditStep(
            'fringe_accommodation',
            'max(0, (monthly_rental × 12 − abatement) / 12)',
            {
              monthlyRentalValue: benefit.monthlyRentalValue,
              abatement,
              furnished: benefit.furnished ?? false,
              furnishedMultiplier: benefit.furnished ? furnishedMultiplier : 1,
            },
            taxable
          ),
        ],
