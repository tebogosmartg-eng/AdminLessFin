...
import type { AuditStep, StatutoryEngineId, StatutoryEngineResult } from './types';
...
export function resolveRebate(
  ...
  ages: { secondaryAge: number; tertiaryAge: number }
): number {
  let rebate = rebates.primary;
  if (age != null && age >= ages.secondaryAge) {
    rebate += rebates.secondary;
  }
  if (age != null && age >= ages.tertiaryAge) {
