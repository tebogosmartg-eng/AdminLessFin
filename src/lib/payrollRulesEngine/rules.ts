...
import { getRuleById } from './catalogue';
import { calculatePaye, normalizeSalaryToMonthly, roundCurrency } from './paye';
import type {
  CalculationState,
  PayrollRulesContext,
  RuleCalculator,
...
export const uifRule: RuleCalculator = {
  ...
  calculate(ctx, state) {
    ...
    const rate = Number(config.rate ?? ctx.taxYearConfig.uifRate);
    if (!Number.isFinite(rate)) {
      throw new Error('UIF rate missing from tax year legislation. Silent rate fallback is prohibited.');
    }
    const ceiling = Number(config.ceiling ?? ctx.taxYearConfig.uifCeilingMonthly);
    if (!Number.isFinite(ceiling)) {
      throw new Error('UIF ceiling missing from tax year legislation. Silent ceiling fallback is prohibited.');
    }
...
export const uifEmployerRule: RuleCalculator = {
  ...
  calculate(ctx, state) {
    ...
    const rate = Number(config.rate ?? ctx.taxYearConfig.uifRate);
    if (!Number.isFinite(rate)) {
      throw new Error('UIF employer rate missing from tax year legislation. Silent rate fallback is prohibited.');
    }
    const ceiling = Number(config.ceiling ?? ctx.taxYearConfig.uifCeilingMonthly);
    if (!Number.isFinite(ceiling)) {
      throw new Error('UIF employer ceiling missing from tax year legislation. Silent ceiling fallback is prohibited.');
    }
    const remuneration = Math.min(state.grossPay, ceiling);
    const amount = roundCurrency(remuneration * rate);
...
export const sdlRule: RuleCalculator = {
  ...
  calculate(ctx, state) {
    ...
    const rate = Number(config.rate ?? ctx.taxYearConfig.sdlRate);
    if (!Number.isFinite(rate)) {
      throw new Error('SDL rate missing from tax year legislation. Silent rate fallback is prohibited.');
    }
